...
// sjcl.js (unminified for easier debugging)
...
sjcl.codec.utf8String = {
  ...
  fromBits: function (arr) {
    ...
    return decodeURIComponent(encodeURIComponent(out)); // PJ
  },
...
sjcl.misc.pbkdf2 = function (password, salt, count, length, Prff) {
  ...
  for (k = 1; 32 * out.length < (length || 1); k++) {
    var block = b.concat(salt, [k]); // PJ
    u = ui = prf.encrypt(block);
...
// The sjcl-scrypt library itself.

importScripts('sjcl-scrypt.js');
...
// Test case definitions.  Includes some variants based on
// RFC6070 and the four test vectors in Percival's original
// scrypt paper.

sjcl.misc.scrypt.tests = [
    {
        "salt": "salt",
        "password": "password",
        "N": 2,
    ...
    /* // This last one crashes every browser I've tried it on. :)
    {
        "salt": "SodiumChloride",
        "password": "pleaseletmein",
        "N": 1048576,
...
function test(testCase) {
  var salt = sjcl.codec.utf8String.toBits(testCase["salt"]);
  var password = sjcl.codec.utf8String.toBits(testCase["password"]);
  var N = testCase["N"];
  var r = testCase["r"];
  var p = testCase["p"];
  var dkLen = testCase["dkLen"];
  var result = sjcl.misc.scrypt(password, salt, N, r, p, dkLen);
  var resultHex = sjcl.codec.hex.fromBits(result);
  if (!(testCase["expected"] === resultHex)) {
    throw new Error("Test case failed: " + JSON.stringify(testCase) + " failed with " + resultHex);
  }
}

function testAll(worker) {
  var tests = sjcl.misc.scrypt.tests;
  for (var i = 0; i < tests.length; i++) {
    worker.postMessage("Running test case #" + i);
    test(tests[i]);
  }
  var end = new Date();
}
...
self.addEventListener('message', function(event) {
  try {
    ...
    testAll(self);
    var end = new Date();
    self.postMessage("All tests passed in " + (end - start) + " ms.");
  } catch (e) {
